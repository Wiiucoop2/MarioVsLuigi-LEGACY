...
public class PlayerController : MonoBehaviourPun, IPunObservable {
    ...
    float jumppTime = 0.5f;
	float sprinttTime = 0.5f;
...
//HACKY WAY OF MAKING USER INPUTS VIA InControl asset
	void Update(){
		if (Application.platform == RuntimePlatform.Switch){
			if (characterActions.Jump.WasPressed)
			{
				//Debug.Log("InControl JJUUMMPPED");
				jumppTime+=0.1f;
				jumpper(jumppTime);
			} else if (characterActions.Jump.WasReleased){
				jumpper(0f);
			}
			
			if (characterActions.Sprint.IsPressed)
			{
				//Debug.Log("InControl SPRINTTED");
				sprinttTime+=0.1f;
				sprintter(jumppTime);
			} else if (characterActions.Sprint.WasPressed){
				sprintter(0f);
				if (!photonView.IsMine) return;
				if (GameManager.Instance.paused) return;
				if (crouching) return;
			...
			{
				//Debug.Log("InControl MOVING");
				if (!photonView.IsMine) return;
				joystick = new Vector2(characterActions.Right, 0f);
			}else if(characterActions.Right.WasReleased){
				joystick = new Vector2(0f, 0f);
			}
			if (characterActions.Up.IsPressed)
			{
				//Debug.Log("InControl UPWARDS");
				if (!photonView.IsMine) return;
				joystick = new Vector2(0f, characterActions.Up);
			}else if(characterActions.Up.WasReleased){
				joystick = new Vector2(0f, 0f);
			}
			
			
			if (characterActions.Select.IsPressed) {
			if (!photonView.IsMine) return;
			if (GameManager.Instance.paused) return;
			if (dead) return;
			if (storedPowerup == null || storedPowerup.Length <= 0) return; 
			...
			if (characterActions.Start.WasReleased) {
				if (!photonView.IsMine) return;
				PlaySoundFromAnim("pause");
				GameManager.Instance.Pause();
			}
